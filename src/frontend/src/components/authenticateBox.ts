...
export const authenticateBoxFlow = async <T, I>({
  ...
  const doLogin = async ({
    ...
    // XXX: this lets subtle crypto throws an exception; this exception should be caught
    // and handled (this happens when the PIN is incorrect)
    const result1 = await withLoader(() =>
      loginPinIdentityMaterial({ userNumber, pin, pinIdentityMaterial })
    );
...
const loginPinIdentityMaterial = async ({
  connection,
  userNumber,
  pin,
  pinIdentityMaterial,
